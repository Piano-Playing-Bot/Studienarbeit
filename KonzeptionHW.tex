...
% @TODO(Val): Bei jedem Unterkapitel sollte der Autor erwähnt werden, weil ihr euch die ja aufgeteilt habt
...
\begin{enumerate}
	\item 	Der Preis muss im Rahmen des selbst gestellten Budgets (< 2000 €) liegen % @TODO(Val): Wann wurde das Budget auf 2000€ begrenzt? Sollte vielleicht im Zielstellung-Kapitel erwähnt werden
...
% @TODO(Val): Fühlt sich an als würde hier noch Text fehlen, oder?


\subsection{Ansteuerungskonzept} \label{konzeptionHW-ansteuerungskonzept}
% @Note(Val): mMn fehlt ein Satz hier oder irgendwann davor, dass Aktuatoren benötigt werden. Ist vielleicht offensichtlich, aber ein Satz der Form: "Zum automatischen Anspielen des Klaviers benötigt es Aktuatoren" oder so.
Das Ansteuerungskonzept für die Tasten eines Klaviers erfordert eine Betrachtung der Befestigungsorte für
die Aktuatoren. Eine grundlegende Überlegung ist, ob die Saiten oder die Tasten selbst angespielt werden sollen. % @Note(Val): Würde vllt. klarer machen, dass es um das direkte Ansteuern der Tasten bzw. Saiten geht, weil die Saiten (indirekt) ja in jedem Fall angespielt werden (sonst gäbe es ja keinen Ton, soweit ich Pianos verstehe)
...
% @Note(Val): Das letzte Argument überzeugt nicht, da beide Versionen das manuelle Spielen einschränken (wenn nicht unmöglich machen) solange es montiert ist

Eine andere Möglichkeit besteht darin, die Aktuatoren unter den Tasten anzubringen.
Die Ansteuerung der Tasten kann dann entweder durch Ziehen mittels
Seilen oder ähnlichem erfolgen oder durch Drücken vom hinteren Ende der Tasten. Beide Optionen ermöglichen es, das Klavier sowohl vom \enquote{Piano Player} % @Note(Val): Piano Player wurde vorher nie als Begriff verwendet - es sollte vorher irgendwann erwähnt werden, dass wir den Hardware-Teil so nennen (ich nehme an, dass sich Piano Player nur auf die HW bezieht oder meint es den gesamten Prototypen?)
...
Tastenempfindlichkeit beeinträchtigen. Das Problem der präzisen Ansteuerung fällt beim Ziehen der Tasten weg. % @Note(Val): Klingt ein bisschen repetetiv mit den vorherigen Sätzen, inhaltlich aber ok
...
% @Note(Val): Warum kommt hier eine Definition von Aktuatoren, wenn das Wort vorher schon frei verwendet wurde
% Entweder setzen wir voraus, dass die Leser:innen wissen, was ein Aktuator ist, oder die Definition muss früher kommen
...
% @Note(Val): Statt arbiträren Beispiel wäre hier eher interessant wie viele Pins wir bräuchten und wie viele wir haben
...
\begin{enumerate}
	...
	unterstützen würden. Zusätzlich dazu lieferten die Simulationen via Tinkercad keine befriedigenden Ergebnisse % @TODO(Val): Footnote mit Link auf Tinkercad einfügen, falls du Tinkercad erwähnst.
...
% @Note(Val): Wir brauchen keine Erklärung von Transistoren, die Leser:innen sollten wissen, was Transistoren sind. Bei Multiplexern davor war die Erklärung zumindest sehr kurz, aber hier hast du mehrere Paragraphen für die Erklärung verwendet. Wenn überhaupt ist hier nur der letzte Teil wichtig: "wie nutzen wir Transistoren"
...
Funktionsweise eines Transistors: % @Note(Val): Du muss nicht jedem Absatz einen Titel geben...
...
% @Note(Val): Entweder machst du eine wirkliche Bullet-List oder du schreibst Sätze statt "<Typ 1>: ... <Typ 2>: ...". Diese Mischung liest sich gerade sehr komisch
...
% @Note(Jay) soll ich Emitter und Collector noch erklären? @Note(Val): Nein. Diese Erklärung ist generell nicht notwendig
...
Unterschied zwischen Transistor und MOSFET: \newline % @Note(Val): Du muss nicht jedem Absatz einen Titel geben...
...
Bei n-MOSFETs (negativ dotierte MOSFETs, welche in diesem Projekt verwendet wurden) wird durch das Anlegen einer % @Note(Val): "in diesem Projekt verwendet wurde" - warum verwenden wir die dann? nicht vorgreifen ist hier wahrscheinlich besser
...
Verwendung von Transistoren in Aktuatoren-Schaltungen: % @Note(Val): Du muss nicht jedem Absatz einen Titel geben...
Transistoren werden in Aktuatoren-Schaltungen verwendet, um die Aktuatoren % @Note(Val): Werden die immer in allen Aktuator-Schaltungen verwendet? Wenn überhaupt ist interessant zu wissen, warum wir Aktuatoren brauchen und das wird hier einfach ignoriert
...
eine ordnungsgemäße Funktion des MOSFETs sicherzustellen und Schäden zu vermeiden. % @Note(Val): Wiederholung des Worts "sicherzustellen"
...
Da der Arduino limitierte \nameref{PWM}-fähige Ausgänge bereitstellt, werde Schieberegister (74HC595) verwendet. % @Note(Val): Warum \nameref statt \ac? "PWM-fähige" ließt sich genauso gut, wenn nicht sogar besser...
% @Note(Val): 'Arduino hat PWM Pins, deshalb verwenden wir Schieberegister' ergibt keinen Sinn. Referenziere doch einfach das vorherige Unterkapitel und sag, dass Schieberegister zusammen mit den PWM Pins verwendet werden, um 88 unterschiedliche PWM Signale zu erhalten
Mit jedem \enquote{in Reihe} geschaltetem Schieberegister kann die Anzahl \ac{PWM}-fähiger Ausgänge um 8 erweitert werden.

\subsubsection{Schieberegister}

% @Note(Val): Müssen die Schieberegister so verbunden werden und wir schreiben das hier nur hin, damit Menschen es leichter nachbauen können? Falls ja, würde ich das kurz erwähnen. Falls nein, muss erklärt werden, warum wir es so und nich anders verbinden
Der Arduino wird an fünf Stellen mit dem ersten Schieberegister verbunden:

% @Note(Val): Das geht mit Sicherheit schöner darzustellen (z.B. mit Tikz). Falls ich genug Zeit und Kraft am Ende habe, kann ich das auch Montag vor der Abgabe noch anpassen, aber falls ihr davor schon Kraft & Zeit dafür habt, wäre das super. Ansonsten ist es so wie es jetzt ist auch ok, auch wenn es nicht wirklich schön ist.
Arduinoport D2 <-> Serial (SER) Input

Über diese Verbindung werden serielle Daten bitweise in das Register geschoben. % @Note(Val): Sind das PWM Sigale? Wenn ja, sollte das vllt erwähnt werden
...
% @Note(Val): Leser:innen wissen wie Schieberegister funktionieren. Der letzte Satz kann mMn weg
...
Das ist besonders relevant, da sonst unter Umständen alle Ausgänge von einer Änderung im letzten Schieberegister bedingt wären. % @Note(Val): Der letzte Satz ergibt keinen Sinn für mich. Willst du sagen, dass sonst eins der 8 Input-Signale bei allen 8 Ausgaberegistern gespeichert werden würde?
...
Für hohe Spannungen und schnelle Schaltvorgänge eigenen sich besonders MOSFETs (Metall-Oxid-Halbleiter-Feldeffekttransistor). % @Note(Val): Verwendet das Abkürzungsverzeichnis...
...
\subsubsection{Hubmagnet} % @Note(Val): Fühlt sich komisch an, einen Titel für einen einzelnen Satz zu haben. Warum brauchn wir diese subsubsections überhaupt? Liest sich auch gut ohne ständige neue Unterüberschriften...

Um den Stromkreis zu schließen wird der andere Kontakt des Hubmagnetes mit dem +24V verbunden.

\subsubsection{Testen}

Um die Fehlersuche zu erleichtern, werden LEDs mit in den Schaltplan eingebaut.
...
% @Note(Val): Der ganze Absatz ist voll von "wir" und "uns" - muss weg
% @Note(Val): Diese Limitierung gehört nicht in die Konzeption
Wie bereits erklärt, wurden insgesamt 88 Aktuatoren verbaut, % @Note(Val): Warum wurden die Aktuatoren bereits verbaut, wenn wir nur in der Konzeption sind. Warum sagen wir nicht einfach: "Es ist geplant, insgesamt 88 Aktuatoren zu verbauen" oder so was
...
eine Stromversorgung von etwa 0.4 Ampere. Bei 10 Aktuatoren sind das also 4 Ampere. Das Netzteil, das wir verwenden, ist auf % @Note(Val): Warum ist das eine Limitierung der Konzeption, wenn einfach ein besseres Netzteil verwendet werden könnte? Klingt mMn eher nach einer Limitierung der Umsetzung
...
% @Note(Val): "Kein Mehrwert"? Der Mehrwert ist, dass mehr Tasten gleichzeitig bespielt werden können. Deswegen nennen wir es ja generell als Limitation... Besser wäre hier zu sagen, dass unser Ziel nur die Erstellung eines Prototypen ist, um herauszufinden wie technisch aufwendig die Entwicklung eines selbstspielenden Klaviers ist und wir das mit dieser Limitation genauso zeigen können

\paragraph{Pedalansteuerung}
% @Note(Val): Wieder viel "wir" und "uns"
Ein Klavier hat normalerweise zwei oder drei Pedale, welche die Dynamik und den Klang des Klaviers beeinflussen.
Diese sind schwerer anzuspielen als die Tasten und benötigen somit Leistungsfähigere Aktuatoren als wir für die Tasten nutzen. % @Note(Val): Mit "schwerer anzuspielen" meinst du, dass es mehr Kraft braucht? Dann sag das lieber direkt. "Die Pedale sind schwerer" oder "brauchen mehr Kraft zum Betätigen", oder so was
...
% @Note(Val): Vllt erwähnen, dass es für die Software interessant wäre, da Pedale gerade von der Software komplett ignoriert werden? Dann würde es aber keinen Sinn ergeben, die Limitation so früh zu erwähnen. Stattdessen könnten wir vielleicht Limitation allgemein später erwähnen?

\paragraph{Wärmeabfuhr}
% @Note(Val): Wieder eine Limitation der Umsetzung nicht der Konzeption
...
% @Note(Val): Hier fehlt ein Grund, warum wir dann keine Metallplatte nutzen
