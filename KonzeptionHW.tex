...
Für die Entscheidung der Aktuatoren wurden vier Arten betrachtet:
\begin{enumerate}
	\item Hubmagnete
	\item Linearmotoren
	\item Servomotoren
...
Die Steuerung erfolgt kann mit oder ohne Multiplexing erfolgen.
...
Ein Multiplexer (Beziehungsweise ein Demultiplexer) ist ein integrierter Schaltkreis, welcher in der Lage ist aus
mehreren Eingangssignalen ein einzelnes zu erstellen beziehungsweise aus einem einzelnen SIgnal mehrere Ausgangssignale zu erstellen.
Der Schaltkreis basiert auf einfachen Logik gattern.
Im Gegensatz zu der LED-Matrix ohne Demultiplexer, werden also weniger Eingänge benötigt.

% TODO(Jay): Nachteile Matrix

%TODO(Jay): Diskussion, dass wir uns für Schieberegister entschieden haben

Für die praktische Konzeption wird ein Schieberegister genutzt. Die LED-Matrix hat zwar nicht das gleiche Latenzproblem
der Schieberegister, aber da die Latenz vernachlässigt werden kann,
%muss ich im Kapitel Schieberegister erklären
wurde die auch bei der Entscheidung nicht weiter betrachtret. Die Schieberegister bieten eine simplere Schaltung als
die Matrix. Außerdem ist die ansteuerung auf Software ebene leichter.
Zudem wurde die Entscheidung auf Basis des Zeitaufwands getroffen. Das Löten der Matrix-Schaltung hätte eine Vielzahl an
Lötstellen benötigt als wir mit den Schieberegistern haben.

%%%%%%%%%%%%%%%%%%%%%%%%
% Er wähne ich dass wir am Anfang einfach noch nicht sicher waren ob die Matrix für die Magnete richtig funktioniert?
% Cause das ganze basiert auf PWM-Signalen. Je nachdem welcher Magnet wie angesteuert wird, dachte ich es kann zu Problemen
% führen weil die Schaltungen sich in die quere kommen. Also wir wussten noch nicht welche Aktuatoren wir nutzen.
% Das hätten wir easy evaluieren können, schien aber unnötig considering dass wir auch Schieberegister nutzen konnten
% bei denen wir aus Erfahrungsberichten schon wussten dass es funktioniert
%%%%%%%%%%%%%%%%%%%%%%%%%
