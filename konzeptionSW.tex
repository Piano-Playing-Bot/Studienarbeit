...
Entsprechend wurde entschieden, die \enquote{Magic Bytes} sowie die IDs der Nachrichten-Typen auf jeweils ein einzelnes Byte zu reduzieren.
Zur leichteren Identifizierung werden Nachrichten-IDs nur ASCII-enkodierte Buchstaben verwendet.
Weiterhin wird hier die Konvention genutzt, dass Großbuchstaben eine Nachricht der \ac{UI} und Kleinbuchstaben eine Nachricht des \ac{MC}s spezifzieren.
...
Wie in den Anforderungen oben erwähnt, benötigt es jedoch noch einer Möglichkeit Fehler in der Kommunikation zu erkennen.
...
Da der Austausch von \enquote{Ping}- und \enquote{Pong}-Nachrichten dem selben Prinzip der Empfangsbestätigung folgt, sollte die Frage aufkommen, warum statt einer eigenen \enquote{Pong}-Nachricht nicht einfach die selbe \enquote{Success}-Nachrich gesendet werden kann.
...
\subsection{Musik-Nachrichten} \label{SPPP-Pidi-Messages}
\begin{enumerate}
    \item Notwendigkeit für Streaming
    \item Umsetzung von Streaming über Double-Buffer, Request-Nachrichten, Indexe in Music-Nachrichten
    \item Kurze Erklärung, warum Empfangsbestätigungen bei Request-Nachrichten nicht notwendig sind
    \item Notwendigkeit eines abstrakt-definierten PlayedKeys
    \item Aufbau der Music-Nachricht (bzw. kurze Erklärung \& Spec in Anhang?)
\end{enumerate}
