...
% @Note(Jay): es ist verwirrend dass du sagst in 2 Teile, aber es gibt 3 separierbare Aufgaben
...
% @Question(Jay): Ich würde vllt "statt Informationen über Geräusche..." oder so schreiben, weils sich für mich durch den zwischensatz nicht mehr direkt auf das vorher gesagte bezieht
Das genaue Geräusch, das am Ende abgespielt werden sollte, wird dabei noch von anderen Faktoren beeinflusst.
So könnten mehrere Noten gleichzeitig zu spielen sein. % @Note(Jay): Ist das eine logische Konsequenz daraus? Cause das kommt für mich nicht automatisch aus dem gesagten raus.
...
% @Note(Jay): find ich okay, ich glaub ich bezieh mich auch öfter nur auf "Projekt"
...
% @Note(Jay): 1 oder 2 Sätze dazu wären sinnvoll
% @Note(Jay): Bei dem "auch wenn" weiß man übrigens nicht so ganz ob du über 1 oder 2 redest
...
%%%%%%%%%%%%%%%%%%
    % @Vorschlag(Jay):
    % Zum Anspielen eines Klaviers werden zu jedem Zeitpunkt... [erklärung die du zu 2 hast]
    % Im Gegensatz dazu würde (1) Informationen direkt speichern, die... [erklärung die du zu 1 hast]
%%%%%%%%%%%%%%%%%%
...
Aus Sicht der Nutzer:innen hat \ac{SAM} zwei entsprechende Aufgaben:
\begin{enumerate}
...
Sollte die Nutzer:in nach dem Import Konfigurationen vornehmen können, ist es sinnvoll eine neue Seite dafür aufzumachen.
