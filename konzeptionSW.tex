...
\begin{enumerate}
    ...
    \item PIDI % @TODO(Val)
    \item Timeline-Jump % @TODO(Val)
    \item Die \ac{UI} muss dem \ac{MC} sagen können, dass die Musik angehalten oder fortgesetzt werden soll
\end{enumerate}

% @TODO(Val): Erweiterte Anforderungen, die hier ignoriert werden
% -> Mehrere Nutzer:innen (z.B. bei kabelloser Verbindung)
% -> Datenverlust hier praktisch kein großes Risiko

% @TODO(Val): Genereller Aufbau von SPPP Nachrichten

% @TODO(Val): Detailierter Aufbau von PIDI Nachrichten

% @Decide(Val): Rest gehört eher ins Umsetzungs-Kapitel oder?
