...
% @Note(Val): Die Reihenfolge ist gerade wahrscheinlich falsch herum für Kruse
% Kruse fände es wahrscheinlich logischer mit der UI zu starten und dann PIDI-Format, MC & Kommunikation zu besprechen
...
% @Note(Val): Check that this is still correct, since I wrote kind of different stuff in the UI section than what I say here
...
Dieses Projekt will explizit eine höhere Performanz als die schnellsten menschlichen Pianist:innen erreichen (siehe Kapitel \ref*{Zielstellung}). % @TODO(Val): Muss im Kapitel auch erwähnt werden

% @Note(Jay): Eine Anforderung an das Projekt ist es anstatt dieses Projekt will?
% @Question(Jay): Macht das bei der Performanz beim spielen an sich einen unterschied oder nur beim "erstellen" der Liste?
...
% @Note(Jay): find ich selbsterklärend
...
% @Note(Jay): Wäre es sinnvoll den Quelltext nach hier zu verschieben?
...
% @TODO(Val): Wiedergabegeschwindigkeit kommt nicht vor, aber wurde bei Anforderungen genannt
% @TODO(Val): Dieses Unterkapitel erklärt größtenteils warum wir die Anforderungen haben und passt deswegen vielleicht besser in Kapitel 2 statt hierhin
...
% @Note(Jay): Zu den anderen Protokollen, ist es wichtig zu erwähnen dass die das auch Nutzen?
...
Diese Planung beschäftigt sich dabei noch nicht direkt mit der Umsetzung oder der Architektur des Codes.
...
% @Question(Val): Bilder von WMP und Spotify einfügen?
% @Note(Jay): Brauchen wir find ich nicht, aber ein Bild von der UI wär glaub ich sinnvoll
...
Weiterhin stellen beide Anwendungen ober oder unterhalb der Timeline auch noch Buttons für die Kontrollwiedergabe dar.
...
% @Note(Jay): Du wechselst oft zwischen Nutzer:in und Nutzer:innen, können wir das vereinheitlichen?
% @TODO(Jay): Gesamt überall einheitlich gendern
Sollte die Nutzer:in nach dem Import Konfigurationen vornehmen können, ist es sinnvoll eine neue Seite dafür zu erstellen.
...
% @Note(Val): Dieses Kapitel nennt eigentlich nur unterschiedliche Möglichkeiten. Wirkliche Entscheidungen habe ich hier nicht getroffen -> sollte wahrscheinlich geändert werden.

% @Note(Val): Mockups unterschiedlicher Design Möglichkeiten wären schön

% @Note(Jay): Ich fänds schon sinnvoll wenn du immer sagst wofür du dich entschieden hast. Ich weiß nicht ob wir Mockups verschiedener Möglichkeiten zwingend brauchen, aber zumindest von dem finalen Resultat sollten wir eins haben.