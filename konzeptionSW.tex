...
Ein Ansatz dafür wäre die Teilung der \ac{UI} in zwei Screens, wobei der erste, im Normalfall angezeigte Screen den Katalog darstellt, und der zweite Screen den Import von Musikstücken behandelt. % @Question(Val): Ist die Verwendung von 'Screen' hier verständlich? Gibt es ein besseres Wort dafür?
Alternativ könnten beide Funktionalitäten auch auf einem einzelnen Screen untegebracht werden mit der Gefahr, die Nutzer:in damit zu überfordern.
Die selbe Frage stellt sich ebenso für die Wiedergabe.
Diese kann entweder auf dem selben Screen mitunter gebracht werden oder auf einen seperaten Screen gebracht werden.
...
Ein eigener Screen wird für die Wiedergabe entsprechend nicht benötigt.
Eine Überlastng des Designs und Überforderung der Nutzer:innen ist hierbei ebenfalls nicht zu erwarten, da diese Nutzer:innen eine solche Aufteilung eines einzelnen Screens bereits von \ac{WMP}, Spotify und ähnlichen Anwendungen gewohnt sind.

Zuletzt ist nun noch das Hinzufügen von Musikstücken zu beachten.


% @TODO(Val): Ablauf für Import
% Datei auswählen (unterstützte Dateiformate) (Fehler möglich)
% Namen spezifzieren (Fehler möglich)
% Abbruch-Möglichkeit

% @TODO(Val): Mockups unterschiedlicher Design Möglichkeiten
