...
Um das Ansprechen der Aktuatoren in regelmäßigen Abständen umzusetzen, ist die Verwendung eines Timer-gesteuerten Interrupts sinnvoll.


Bei Auftreten des Interrupts, sollten dann Daten aus einem Buffer an die Schieberegister weitergeleitet werden.
Auch ist wichtig, die Ausgaben der Schieberegister zu schließen, bis die nächsten Daten durchgeschoben wurden.

Beim \ac{MC}, der in Kapitel \ref{Ansteuerung} gewählt wurde, müssen dafür jeweils nur die Daten an die richtigen Pins geschrieben werden.
Damit bei jedem Interrupt, die Daten an die Pins geschrieben werden können, müssen diese Daten vorher in einer globalen Liste vorbereitet werden.
Bei jedem Interrupt können die Werte, die zu jedem Aktuator geschickt werden sollen, dann einfach aus der Liste gelesen und an die Pins geschrieben werden.
